...
class Server(object):
    def __init__(self, args, times):
        ...
        self.global_model = copy.deepcopy(args.model)  # 获取当前轮次的全局模型
    ...
    def target_clients(self): # 设置投毒客户端
        self.target_id = [0]
        if self.args.mode != 7:
    ...
    def save_global_model(self):
        ...
        model_path = os.path.join(model_path, str(self.mode) + "_" + self.algorithm + "_server" + "_" + str(
            self.args.target_label) + "_" + str(self.args.poi) + ".pt")
    ...
    def save_init_global_model(self):
        ...
        if not os.path.exists(model_path):
            os.makedirs(model_path)
        model_path = os.path.join(model_path, "init" + "_" + self.algorithm + "_server" + "_" + str(
            self.args.target_label) + "_" + str(self.args.poi) + ".pt")
        torch.save(self.global_model, model_path)

    def load_model(self):
        model_path = os.path.join("models", self.dataset)
        model_path = os.path.join(model_path, str(0) + "_" + self.algorithm + "_server" + "_" + str(
            self.args.target_label) + "_" + str(self.args.poi) + ".pt")
    ...
    def train_metrics(self): # never used
        acc_num = []
        num_samples = []
        losses = []
    ...
    # def load_poi_test(self):
    #     if self.args.poi:
    #         poi_test = load_npz(f'./data/{self.dataset}/poi_test_{self.args.target_label}_{self.args.ratio}.npz')
    #     return DataLoader(dataset=poi_test, batch_size=self.batch_size, drop_last=False, shuffle=False)
    ...
    def model_eval(self, dataloader):
        ...
        with torch.no_grad():
            for batch_id, batch in enumerate(dataloader):
                ...
                # predicted_labels.extend(torch.argmax(output, dim=1).cpu().numpy())
        test_acc = 1.0 * (float(test_acc) / float(test_num))
    ...
    def evaluate(self, acc=None, loss=None):
        ...
        targetacc, _ = self.model_eval(target_loader)
        print("Target acc: {:.4f}".format(targetacc))

        # 测试集
        test_loader = self.load_test_data()
        testacc, test_loss = self.model_eval(test_loader)
